...
const PrintCategoryProd = (req, res) => {
  ...
  const  query = url.parse(req.url, true).query;
       ...
       if (req.session.auth)   {   // (로그인된 경우에만) 상품리스트를 출력합니다
           ...
           sql_str = "SELECT itemid, maker, pname, modelnum, rdate, price, dcrate, amount, pic, pdesc from u21_products where category='" + search_cat + "' order by rdate desc;"; // 상품조회SQL
